...
/* ==========================================================
   ...
   ----------------------------------------------------------
   This service is intentionally self-contained. It must not
   depend on optional testNormalization/testIdentity modules.
   ========================================================== */
...
const PANEL_NAME_SET = new Set([
  ...
  "flp", "flp panel", "fasting lipid profile", "full lipid profile", "complete lipid profile", "lipid profile",
  "cbc", "cbc panel", "fbc", "full blood count", "complete blood count", "full blood count panel",
  "coagulation profile", "coagulation panel", "coagulation studies", "coagulation screen",
  "iron profile", "iron studies", "iron panel", "iron studies profile",
  "ue", "ues", "uecs", "eucr", "urea electrolytes", "urea and electrolytes", "urea & electrolytes", "serum electrolytes",
  "serum bilirubin", "bilirubin profile", "bilirubin panel",
  "serum calcium", "calcium profile", "calcium panel",
  "glucose profile", "ogtt", "ogtt panel", "oral glucose tolerance test"
]);

const PANEL_CANONICAL = {
  "lft": "liver function test", "lft panel": "liver function test", "liver function profile": "liver function test", "liver profile": "liver function test",
  "rft": "renal function test", "rft panel": "renal function test", "renal function profile": "renal function test", "renal profile": "renal function test",
  "flp": "lipid profile", "flp panel": "lipid profile", "fasting lipid profile": "lipid profile", "full lipid profile": "lipid profile", "complete lipid profile": "lipid profile",
  "cbc": "full blood count", "cbc panel": "full blood count", "fbc": "full blood count", "complete blood count": "full blood count", "full blood count": "full blood count", "full blood count panel": "full blood count",
  "ue": "serum electrolytes", "ues": "serum electrolytes", "uecs": "serum electrolytes", "eucr": "serum electrolytes", "urea electrolytes": "serum electrolytes", "urea and electrolytes": "serum electrolytes", "urea & electrolytes": "serum electrolytes",
  "serum bilirubin": "serum bilirubin", "bilirubin profile": "serum bilirubin", "bilirubin panel": "serum bilirubin",
  "serum calcium": "serum calcium", "calcium profile": "serum calcium", "calcium panel": "serum calcium",
  "ogtt": "oral glucose tolerance test", "ogtt panel": "oral glucose tolerance test"
};

const getCanonicalTestName = (value) => identityNormalize(value);
const getCanonicalPanelName = (value) => {
  const n = identityNormalize(value);
  return PANEL_CANONICAL[n] || n;
};
const getCanonicalPanelIdentity = getCanonicalPanelName;
const isKnownPanelName = (value) => PANEL_NAME_SET.has(identityNormalize(value)) || PANEL_CANONICAL[identityNormalize(value)] !== undefined;

/* IMPORTANT: Panel Test is a legitimate child classification.
   Only actual panel records are rejected as panel parameters. */
const isPanelTest = (row = {}) => {
  if (!row || typeof row !== "object") return false;
  const type = identityNormalize(row.test_type ?? row.testType ?? row.result_type ?? row.resultType);
  return row.is_panel === true || row.isPanel === true || type === "panel" || type === "panel profile" || identityNormalize(row.result_type ?? row.resultType) === "panel";
};

const getRelatedPanelId = (row = {}) => firstIdentityValue(
  row.panel_id, row.panelId, row.parent_panel_id, row.parentPanelId,
  row.masterTest?.panel_id, row.master_test?.panel_id
);

const normalizeTestName = (value) => identityNormalize(value);
...
/* ==========================================================
   COMPATIBILITY REFERENCE RANGE EXPORT
   ----------------------------------------------------------
   Existing ChemistryPanelResultEntry imports this named helper.
   Keep it here so the unified service remains backward-compatible.
   No reference values are invented; the parameter's configured
   sex/age/general metadata is used in priority order.
   ========================================================== */

export function resolveReferenceRange(parameter = {}, demographics = {}) {
  ...
  const ageRaw = demographics?.age ?? demographics?.patient_age ?? parameter?.age;
  const age = Number(ageRaw);

  const female = firstIdentityValue(
    parameter?.female_range,
    parameter?.femaleRange
  );
  const male = firstIdentityValue(
    parameter?.male_range,
    parameter?.maleRange
  );
  const child = firstIdentityValue(
    parameter?.child_range,
    parameter?.childRange
  );
  const elderly = firstIdentityValue(
    parameter?.elderly_range,
    parameter?.elderlyRange
  );
  const reference = firstIdentityValue(
  ...
  if (Number.isFinite(age) && age < 18 && child) return String(child).trim();
  if (Number.isFinite(age) && age >= 65 && elderly) return String(elderly).trim();
  if (["female", "f"].includes(sex) && female) return String(female).trim();
  if (["male", "m"].includes(sex) && male) return String(male).trim();

  return String(reference || male || female || child || elderly || "").trim();
}
