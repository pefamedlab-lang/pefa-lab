/* ==========================================================
   ...
   ----------------------------------------------------------
   Dedicated quantitative panel for neonatal / paediatric
   bilirubin assessment.

   Features
   - Total, Direct, Indirect Bilirubin
   - DOB-based postnatal age calculation
   - Optional birth-time/hour precision
   - Sample-collection date/time aware reference selection
   - Automatic age-specific reference range + flag
   - Indirect bilirubin = Total - Direct
   - Edit Mode hydration
   - Preserves the existing structured panel payload contract
   ========================================================== */
...
/* ==========================================================
   REGISTRATION DEMOGRAPHIC SOURCE
   ----------------------------------------------------------
   The registration selected in LaboratoryResultEntry is the
   authoritative demographic source for Serum Bilirubin.
   This prevents a stale patient object from showing a different
   DOB from the one used during registration.
   ========================================================== */
const getRegistrationDob = (registration, patient, sourceResult) =>
  firstValue(
    ...
    sourceResult?.dob,
    sourceResult?.date_of_birth,
    sourceResult?.dateOfBirth
  );

const patientValue = (patient, registration, key) =>
  firstValue(
    registration?.[key],
    registration?.patient?.[key],
    patient?.[key],
    patient?.patient?.[key]
  );
...
const getCollectionInput = (source, registration, patient) => {
  const raw = firstValue(
    source?.sample_collection_date_time,
    source?.sampleCollectionDateTime,
    source?.specimen_collection_time,
    source?.specimenCollectionTime,
    source?.collection_datetime,
    source?.collectionDateTime,
    registration?.sample_collection_date_time,
    registration?.sampleCollectionDateTime,
    patient?.sample_collection_date_time,
    patient?.sampleCollectionDateTime
  );
...
export default function SerumBilirubinPanelResultEntry({
  ...
  const neonatalContext = useMemo(() => {
    const collectionIso = toIsoFromLocalInput(collectionDateTime);
  ...
  const publishChange = useCallback(
    (nextValues) => {
      ...
      const nextIndirect = calculateIndirectBilirubin(
        nextValues.total_bilirubin,
        nextValues.direct_bilirubin
      );

      const next = {
        ...nextValues,
        indirect_bilirubin: nextIndirect,
      };
      ...
      onChange({
        ...
        neonatal_bilirubin: {
          ...
          birth_datetime: neonatalContext.birthDateTime?.toISOString?.() || neonatalContext.birthDateTime || null,
    ...
    [onChange, patient, registration, test, sourceResult, collectionDateTime, neonatalContext, editMode]
  );
    ...
    <div className="pefa-serum-bilirubin-panel" style={styles.shell}>
      ...
      <div style={styles.contextGrid}>
        <div style={styles.contextCard}>
          ...
          <div><span>DOB</span><strong>{dob ? formatDate(dob) : "Not available"}</strong></div>
        </div>
        <div style={styles.contextCard}>
          <Clock3 size={16} />
          <div><span>Birth time</span><strong>{birthTime || "Not recorded"}</strong></div>
        </div>
        <div style={styles.contextCard}>
          <CalendarDays size={16} />
          <div><span>Postnatal age</span><strong>{neonatalContext.age?.display || "Awaiting DOB"}</strong></div>
        </div>
      </div>
      ...
      <div style={styles.collectionRow}>
        <label style={styles.collectionLabel}>
          ...
          <input
            ...
            onChange={(event) => {
              setCollectionDateTime(event.target.value);
        ...
        <div style={styles.infoBox}>
          ...
          <div>
            ...
            <span>
              The selected laboratory reference interval is determined from DOB and the sample collection time.
            </span>
          </div>
        </div>
      </div>
        ...
        <table style={styles.table}>
          <thead>
            <tr>
              ...
              <th style={styles.th}>Age-specific reference</th>
          ...
          <tbody>
            {displayRows.map((row) => (
              <tr key={row.key}>
                <td style={styles.td}><strong>{row.name}</strong>{row.calculated && <small style={styles.calculated}>Calculated</small>}</td>
                <td style={styles.td}>
                  <input
                    type="number"
                    step="0.01"
                ...
                <td style={styles.td}>
                  <span className={flagClass(row.flag)}>{row.flag || "—"}</span>
                </td>
              </tr>
            ))}
          </tbody>
