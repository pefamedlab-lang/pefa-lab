...
export default function StaffManagement() {
  ...
  const saveStaff = async () => {
    ...
    if (
      ...
      fail(
        "Password is required when creating a staff account."
      );
    ...
    console.group(
      "%c[PEFA STAFF] Saving staff record",
      "color:#0b63ce;font-weight:bold;"
    );
    ...
    try {
      ...
      /*
       * Preserve existing password during edit.
      ...
      console.log(
        "Staff save payload:",
        {
          ...
          /*
           * Do not expose password in console.
           */
          password:
            payload.password
              ? "********"
      ...
      console.log(
        "STEP — Verifying saved media fields..."
      );
      ...
      const {
        ...
        await supabase
          .from("staff_users")
          .select(
            "id, full_name, profile_photo, signature_url"
          )
          .eq(
            "id",
            savedRow.id
      ...
      console.log(
        "Verified profile_photo:",
        verified?.profile_photo
      );

      console.log(
        "Verified signature_url:",
        verified?.signature_url
      );
      ...
      if (
        ...
        throw new Error(
          "Staff record saved, but the profile photo URL was not retained in staff_users."
        );
      }
      ...
      if (
        ...
        throw new Error(
          "Staff record saved, but the signature URL was not retained in staff_users."
        );
      }
      ...
      notify(
        editingStaff
          ? "Staff updated successfully. Profile and signature were retained."
          : "Staff created successfully. Profile and signature were retained."
      );
      ...
      console.log(
        "🎉 STAFF SAVE COMPLETED SUCCESSFULLY."
      );
    } catch (error) {
      console.error(
        "❌ [STAFF MANAGEMENT] saveStaff:",
        error
      );
  ...
  const resetPassword = async (
    ...
    try {
      ...
      console.error(
        "[STAFF MANAGEMENT] resetPassword:",
        error
      );
