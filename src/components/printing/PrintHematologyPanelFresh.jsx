/*
 * PEFA ENTERPRISE LIS
 * PrintHematologyPanelFresh.jsx
 *
 * Dedicated renderer for HematologyPanelResultEntry.jsx.
 *
 * CBC RULE:
 * MCV, MCH and MCHC are rendered from the saved analyser/manual value.
 * This printer NEVER recalculates them.
 */
...
export default function PrintHematologyPanelFresh(props) {
  const {
    report = {}, rows,
  ...
  return (
    <PEFAFormalReportShell
      report={report}
      title={first(title, report.panel_name, report.panelName, report.test_name, report.testName, "HAEMATOLOGY REPORT")}
    ...
    >
      <table className="pefa-results-table">
        ...
        <tbody>
          {parameters.map((p, i) => {
            // Deliberately use the persisted value only. No CBC formula is applied here.
